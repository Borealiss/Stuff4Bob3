...
ENT.Type            = "anim"
// PROJECT RENEGADE FOREVER
ENT.FireStraight = false
ENT.DMG = 250
ENT.DMGDist = 500
ENT.Colors = {r=25,g=50,b=255}
ENT.ExplodeSound = "renegadelaat/missilepop"..math.random(2,3)..".wav"
...
if SERVER then
	...
	function ENT:FollowTarget( followent )
		local speed = self:GetStartVelocity() + (self:GetDirtyMissile() and 5000 or 6000)
	...
	function ENT:Initialize()	
		...
		self.snd = CreateSound(self, "renegadelaat/torpedoloop.wav")
		
		self.snd:PlayEx(1,math.random(100,110))
	end
	...
	function ENT:OnRemove()
		...
		sound.Play(self.ExplodeSound, Pos, 130, math.random(90,110), 1 )
	...
	function ENT:Explosion( pos )
		...
		for i = 0,60 do
			...
			if particle then
				...
				particle:SetEndSize( math.Rand(120,250) )
				particle:SetRoll( math.Rand(-1,1) )
				particle:SetColor( 50,50,50 )
		...
		for i = 0, 40 do
			...
			if particle then
				particle:SetVelocity( VectorRand(-1,1) * 1500 )
				particle:SetDieTime( 0.14 )
				particle:SetStartAlpha( 255 )
				particle:SetStartSize( 10 )
				particle:SetEndSize( math.Rand(110,150) )
				particle:SetEndAlpha( 100 )
				particle:SetRoll( math.Rand( -1, 1 ) )
				particle:SetColor( 200,150,250 )
				particle:SetCollide( false )
			end
		end
